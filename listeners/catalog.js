...
import * as errorCodes from "../helpers/errorCodes.json";
...
export const validateCatalogRecords = async (records, event) => {
  ...
  if (
    event.context.slugs.sheet !== "catalog" ||
    (event.context.actorId && event.context.actorId.includes("key"))
  ) {
    console.log({ context: event.context });
    // return;
  }
  const workbook = await api.workbooks.get(event.src.context.workbookId);
  const catalogSheetId = workbook.data.sheets.find(
    (sheet) => sheet.config.slug === "catalog"
  ).id;
  ...
  const res = records.map((record) => {
    ...
    variationKeys.forEach((key) => {
      if (record.get(key) && record.get(key).split("=").length !== 2)
        record.addError(key, errorCodes["g-302"]);
    });
    ...
    var sellPrice = record.get("sellPrice")?.toString();
    if (Number.parseFloat(sellPrice) < 0)
      record.addError("sellPrice", errorCodes["v-003"]);

    var buyPrice = record.get("buyPrice")?.toString();
    if (Number.parseFloat(buyPrice) < 0)
      record.addError("buyPrice", errorCodes["v-003"]);

    var tax = record.get("tax")?.toString();
    if (Number.parseFloat(tax) < 0 || Number.parseFloat(tax) > 100)
      record.addError("tax", errorCodes["v-007"]);

    const recordsWithSameName = records.filter(
      (r) => record.get("name") && r.get("name") === record.get("name")
    );

    if (!hasVariation(record) && recordsWithSameName.length > 1)
      record.addError("name", errorCodes["g-040"]);
    if (hasVariation(record)) {
      notVariationKeys.forEach((key) =>
        recordsWithSameName.every((rcrd) => {
          if (rcrd.get(key) !== record.get(key))
      ...
      if (recordsWithSameOptions.length > 1)
        variationKeys.forEach((key) =>
          record.addError(key, errorCodes["g-031"])
        );
    }
  ...
  recordsUpdates.forEach((record) => {
    Object.entries(record.values).forEach(([key, cell]) => {
      // @ts-ignore
      if (!cell.value) {
...
export const addVariationAction = async (event) => {
  ...
  if (response.data) {
    ...
    throw new Error(
      "Une erreur est survenue lors de la création de la colonne"
    );
  }
};
